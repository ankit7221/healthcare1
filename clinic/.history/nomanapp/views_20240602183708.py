...
def get_quote(request):
    if request.method == 'POST':
        ...
        if quote_form.is_valid():
            quote_form.save()
            return JsonResponse({'message': 'Thank you for your submission! We will get back to you soon.'}, status=200)
        else:
            return JsonResponse({'message': 'Invalid form data.'}, status=400)
    return JsonResponse({'message': 'Invalid request method.'}, status=400)
def about(request):
    testimonials = Quote.objects.exclude(testimonial__isnull=True).exclude(testimonial__exact='')
